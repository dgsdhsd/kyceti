...
int fun(MYSQL* sock,char *name) {
	...
	int sum;
	srand(time(NULL));
	fuzhu(sent, 14, name, mum);                          //16指!的位置
	if (!mysql_query(sock, mum))
		cout << "true" << endl;
	else cout << "flase" << endl;
	result= mysql_store_result(sock);
...
int answ(MYSQL* sock,int sel, int pp[10][105],char* name) {
	MYSQL_RES* result;
	MYSQL_ROW row;
	int i, j;
	char opt[1000];
	memset(opt, 'S', sizeof(opt));
	char sent[100] = "select * from !";
	char mum[100];
	fuzhu(sent, 14, name, mum);
	if (!mysql_query(sock, mum))
		cout << "true" << endl;
	else cout << "flase" << endl;
	result = mysql_store_result(sock);
...
int look(MYSQL* sock, char* name) {
	...
	char sent[100] = "select * from work!";
	char mum[100];
	fuzhu(sent, 18, name, mum);
	if (!mysql_query(sock, mum))
		cout << "true" << endl;
	else cout << "flase" << endl;
	result = mysql_store_result(sock);
	...
	cout << "what the number answer you want:";
	cin >> sel;
	answ(sock,sel, pp,name);
	getchar();
	getchar();
	getchar();
	return 0;
